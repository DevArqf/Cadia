...
import { Shield, Swords, Trophy, Sparkles, ChevronRight, Heart, Code, Users } from 'lucide-react';
	...
	return (
		<div className="relative min-h-screen overflow-hidden cadia-bg scanlines">
			...
			<header className="relative z-10 flex items-center justify-between px-4 sm:px-8 py-4 sm:py-5 border-b border-border/40 backdrop-blur-xl bg-card/30">
				...
				<Button onClick={() => setView('landing')} variant="ghost" size="sm" className="text-xs text-muted-foreground hover:text-foreground">
					← Back to Home
				</Button>
			</header>
			...
			<main className="relative z-10 max-w-5xl mx-auto px-4 sm:px-8 py-12 sm:py-16">
				...
				>
					...
					<h1 className="font-pixel text-2xl sm:text-4xl text-cadia text-glow-cadia mb-3 tracking-wider">ABOUT CADIA</h1>
					<p className="text-sm sm:text-base text-muted-foreground max-w-xl mx-auto leading-relaxed">
						The ultimate RPG and Utility Discord bot. Built for a fun and engaging experience.
					</p>
				</motion.div>
					...
					{SECTIONS.map((section, i) => {
						...
						return (
							<motion.div
								...
								transition={{ duration: 0.5 }}
								className={`flex flex-col ${isImageRight ? 'sm:flex-row' : 'sm:flex-row-reverse'} items-center gap-8 sm:gap-10`}
							>
								{/* Text side */}
								<div className="flex-1 text-left">
									...
									<h2 className="font-pixel text-base sm:text-lg mb-4 tracking-wide" style={{ color: section.color }}>
										{section.title.toUpperCase()}
									</h2>
									<p className="text-sm sm:text-base text-foreground/80 leading-relaxed">{section.text}</p>
								</div>
								...
								>
									{/* Placeholder icon */}
									<div className="flex flex-col items-center gap-3 opacity-50" style={{ color: section.color }}>
										<section.icon className="h-12 w-12" />
										<span className="text-[10px] font-pixel tracking-wider">IMAGE</span>
									</div>
				...
				>
					...
					<p className="text-xs text-muted-foreground mt-3">Free forever. Premium available for power users.</p>
				</motion.div>
			</main>

...
function getDoc(page: "tos" | "faq" | "privacy" | null): LegalDoc | null {
  if (page === "tos") return TERMS_OF_SERVICE;
...
export function LegalView() {
  const legalPage = useCadia((s) => s.legalPage);
  const setLegalPage = useCadia((s) => s.setLegalPage);
  const doc = getDoc(legalPage);

  const [highlight, setHighlight] = useState(false);
  const tosRef = useRef<HTMLButtonElement>(null);
  ...
  return (
    <div className="min-h-screen flex flex-col cadia-bg scanlines">
      ...
      {/* Header */}
      <header className="relative z-10 border-b border-border/60 px-4 py-3 flex items-center justify-between bg-card/60 backdrop-blur-xl sticky top-0 z-30">
        <button
      ...
      <div className="relative z-10 px-4 py-3 flex items-center gap-2 border-b border-border/40 bg-card/30 backdrop-blur-sm sticky top-[49px]">
        ...
        <button
          ...
          } ${highlight ? "cadia-legal-highlight" : ""}`}
        >
          Terms of Service
        </button>
        <button
      ...
      {/* Content */}
      <main className="relative z-10 flex-1 px-4 py-8 sm:py-12 max-w-3xl w-full mx-auto">
        <motion.div
